...
const StreakChart = () => {
  ...
  const streakData = ["daily", "weekly", "monthly"].map((frequency) => ({
    name: frequency.charAt(0).toUpperCase() + frequency.slice(1),
    streak: habits.filter((habit) => habit.frequency === frequency && habit.isDone).length,
  }));

  const progressData = ["daily", "weekly", "monthly"].map((frequency) => ({
    name: frequency.charAt(0).toUpperCase() + frequency.slice(1),
    done: habits.filter((habit) => habit.frequency === frequency && habit.isDone).length,
    notDone: habits.filter((habit) => habit.frequency === frequency && !habit.isDone).length,
  }));
